...
trait DefaultMakeApiHttpClientComponent extends MakeApiHttpClientComponent with TimeInstances {
  ...
  final class DefaultMakeApiHttpClient extends HttpClient {
    ...
    override def post[ENTITY](apiEndpoint: String,
                              urlParams: Seq[(String, Any)] = Seq.empty,
                              data: InputData = "",
                              headers: Map[String, String] = Map.empty,
                              includeContentType: Boolean = true)(implicit decoder: Decoder[ENTITY]): Future[ENTITY] = {
      val requestData = RequestData(
        method = "POST",
        url = urlFrom(apiEndpoint, urlParams),
        data = data,
        timeout = maxTimeout,
        withCredentials = withCredentials,
        headers = defaultHeaders.filter(h => h._1 != "Content-Type" || includeContentType) ++ headers
      )
    ...
    private def askForAccessTokenSocial(provider: String,
      ...
      post[Token](
        "user" / "login" / "social",
        data = Map("provider" -> provider, "token" -> token, "country" -> "FR").asJson
