...
object CreateTopIdea {

  case class CreateProps(questionId: String) extends RouterProps
  case class CreateState(ideaList: Seq[Idea])
  ...
  private lazy val reactClass: ReactClass =
    React
      .createClass[CreateProps, CreateState](
        displayName = "CreateTopIdea",
        getInitialState = { _ =>
          CreateState(ideaList = Seq.empty)
        },
        ...
        render = { self =>
          ...
          <.Create(^.resource := Resource.topIdeas, ^.location := self.props.location, ^.hasList := true)(
            <.SimpleForm()(
              ...
              <.DependentInput(^.dependsOn := "questionId")(
                <.SelectInput(
                  ^.source := "ideaId",
                  ^.label := "idea",
                  ^.translateLabel := ((label: String) => label),
              ...
              <.NumberInput(^.source := "weight", ^.validate := required, ^.options := Map("fullWidth" -> true))()
            )
          )
        }
      )
